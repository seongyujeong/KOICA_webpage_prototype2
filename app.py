...
# 3. 로컬 파일 로드 및 JS 데이터 직접 주입
current_dir = os.path.dirname(os.path.abspath(__file__))

# 🔴 필수: index.html 파일을 먼저 읽어와서 html_content 변수를 만듭니다!
with open(os.path.join(current_dir, "index.html"), "r", encoding="utf-8") as f:
    html_content = f.read()

# 🔴 필수: style.css 파일을 읽어와서 css_content 변수를 만듭니다!
with open(os.path.join(current_dir, "style.css"), "r", encoding="utf-8") as f:
    css_content = f.read()

# 🔴 우리가 직접 주입하는 6단계 자바스크립트 데이터
js_content = """
window.CHARACTERS = [
  {
    id: "abebe",
    ...
    endingKOICA: "KOICA는 에티오피아를 포함한 동아프리카 지역에 '기후스마트 농업 기술'을 전수하고 지하수 관정을 개발하여, 아베베와 같은 농부들이 기후 재난 속에서도 자립할 수 있는 단단한 방패를 만들고 있습니다."
  },
  {
    id: "fatima",
    name: "파티마",
    ...
    endingKOICA: "KOICA는 방글라데시 취약 지역에 '홍수 회복력이 있는 식수 위생 시설(보건 ODA)'을 건립하고 재난 예측 인프라를 구축하여, 파티마와 같은 소녀들이 재난 속에서도 배움을 잃지 않고 안전하게 성장할 수 있도록 지원합니다."
  },
  {
    id: "tane",
    name: "타네",
    ...
    endingKOICA: "KOICA는 기후변화 취약 도서국들을 위해 해안 맹그로브 숲 복원 사업을 지원하고 기후 재난 조기경보 시스템을 전수하여, 타네와 같은 태평양 주민들이 삶의 터전을 잃지 않고 동등한 삶을 누릴 수 있도록 '기후 정의'를 실천하고 있습니다."
  }
];
"""
